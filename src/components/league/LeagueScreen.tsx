...
type Row = { username: string; points: number; played: number; maalloes_total: number; xi_solved: number; finn_points: number };
...
export function LeagueScreen() {
  ...
  return <div className="flex flex-col gap-4">
    ...
    <section className="card overflow-hidden">
      <div className="border-b border-line p-4"><h2 className="font-display text-2xl font-bold uppercase">Sammenlagt siste 30 dager</h2><p className="text-xs text-mist">Mangler XI belønner spillere funnet og færre forsøk. Målløs belønner lav totalsum. Finn spilleren følger hintpoengene.</p></div>
      {rows.length ? <div className="overflow-x-auto"><table className="w-full text-sm"><thead className="text-left text-xs uppercase text-mist"><tr><th className="p-3">#</th><th className="p-3">Spiller</th><th className="p-3 text-right">Poeng</th><th className="p-3 text-right">Spill</th></tr></thead><tbody>{rows.map((r, i) => <tr key={r.username} className={`border-t border-line ${user?.username === r.username ? "bg-sky/10" : ""}`}><td className="p-3 font-display text-lg">{i + 1}</td><td className="p-3 font-semibold">{r.username}</td><td className="p-3 text-right font-display text-xl font-bold text-gold">{r.points}</td><td className="p-3 text-right text-mist">{r.played}</td></tr>)}</tbody></table></div> : <p className="p-5 text-mist">Ingen registrerte resultater ennå. Bli den første på tabellen.</p>}
    </section>
  </div>;
}
