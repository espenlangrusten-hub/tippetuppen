...
export function LeagueScreen() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const r = await apiGet<{ ok: boolean; rows: Row[]; me: Row | null; registered: number; month?: LeagueMonth; champion?: Champion }>("/leaderboard");
      if (!r.ok) throw new Error("Leaderboard unavailable");
      if (request !== boardRequest.current) return;
      setRows(r.rows); setMe(r.me ?? null); setMonth(r.month ? { month: r.month, champion: r.champion ?? null } : null); setRegistered(typeof r.registered === "number" ? r.registered : null); setBoardStatus("ready");
    ...
    <section className="card overflow-hidden">
      ...
      {boardStatus === "loading" ? <p className="p-5 text-mist" role="status">Henter ligaen …</p>
        ...
        : <>
          ...
          </table></div> : <p className="p-5 text-mist">Ingen poeng i {copy ? copy.month : "denne måneden"} ennå. Bli den første på tabellen.</p>}
