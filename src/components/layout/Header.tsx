...
export function Header() {
  ...
  return <>
    ...
    {path === "/" && <nav className="stadium-mobile-nav" aria-label="Mobilmeny">{[...links.slice(0,3),{href:"/liga/#login",name:"Profil",icon:"♙"}].map((l) => <Link key={l.name} href={l.href} aria-current={path === l.href ? "page" : undefined}><span aria-hidden="true">{l.icon === "⚽" ? <FootballIcon /> : l.icon}</span>{l.name}</Link>)}</nav>}
  </>;
}
